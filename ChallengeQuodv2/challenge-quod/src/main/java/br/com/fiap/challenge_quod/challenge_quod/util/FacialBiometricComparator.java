...
public class FacialBiometricComparator {
    ...
    private static final boolean FORCE_FRAUD_TEST = false; // Para simular fraude no fuso horário
    ...
    static {
        try {
            ...
            System.load(libPath);
            // logger.info("Biblioteca nativa do OpenCV compartilhada com sucesso. Versão: {}", Core.getVersionString());
    ...
    public static class ImageComparisonResult {
        ...
        public ImageComparisonResult(boolean facesEqual, boolean coordinatesEqual, double similarityScore, String detailedReason, double lat1, double lon1, double lat2, double lon2, double euclideanDistance, String fraudType) {
    ...
    public static ImageComparisonResult compareImages(String neutralImagePath, String smileImagePath, double lat1, double lon1, double lat2, double lon2) {
        ...
        // logger.info("Diferença de coordenadas - latDiff={}, lonDiff={}, areCoordinatesEqual={}, FORCE_FRAUD_TEST={}", latDiff, lonDiff, areCoordinatesEqual, FORCE_FRAUD_TEST);
        ...
        // logger.info("Imagens investidas com sucesso. Tamanho Neutra: {}x{}, Tamanho Sorriso: {}x{}", neutralImg.cols(), neutralImg.rows(), smileImg.cols(), smileImg.rows());
        ...
        if (neutralDetection.face == null || smileDetection.face == null) {
            // logger.warn("Nenhuma face bloqueada em uma ou ambas as imagens.");
            neutralImg.release();
            smileImg.release();
            return new ImageComparisonResult(false, areCoordinatesEqual, 0.0, "Face não detectada em uma ou ambas as imagens", lat1, lon1, lat2, lon2, 0.0, "Nenhuma Face Detectada");
        }
        ...
        if (neutralEmbedding == null || smileEmbedding == null) {
            ...
            neutralImg.release();
            smileImg.release();
            return new ImageComparisonResult(false, areCoordinatesEqual, 0.0, "Erro ao gerar embeddings faciais", lat1, lon1, lat2, lon2, 0.0, "Erro de Embedding");
        }
        double norm1 = calculateNorm(neutralEmbedding);
        double norm2 = calculateNorm(smileEmbedding);
        // logger.debug("Norma de incorporação neutra: {}, sorriso: {}", norm1, norm2);
        if (norm1 < 0.1 || norm2 < 0.1) {
            // logger.warn("Incorporação inválido detectado: norma neutra={}, norma sorriso={}", norm1, norm2);
            neutralDetection.face.release();
            neutralDetection.alignedFace.release();
            neutralDetection.normalizedFace.release();
            smileDetection.face.release();
            ...
            neutralImg.release();
            smileImg.release();
            return new ImageComparisonResult(false, areCoordinatesEqual, 0.0, "Incorporação inválida (norma muito baixa)", lat1, lon1, lat2, lon2, 0.0, "Incorporação inválida");
        }
        ...
        if (similarityScore >= SIMILARITY_THRESHOLD) {
            ...
            facialReason = "Faces consideradas da mesma pessoa (pontuação: " + similarityScore + ", distância: " + euclideanDistance + ")" + (ssimWarning ? " [Aviso: SSIM baixo: " + ssimScore + "]" : "");
        } else if (similarityScore < FRAUD_THRESHOLD) {
            areFacesEqual = false;
            fraudType = "Rostos Diferentes";
            facialReason = "Faces consideradas de pessoas diferentes (pontuação: " + similarityScore + ", distância: " + euclideanDistance + ")" + (ssimWarning ? " [Aviso: SSIM baixo: " + ssimScore + "]" : "");
        } else {
            areFacesEqual = false;
            fraudType = "Possível Fraude Facial";
            facialReason = "Possível fraude detectada (pontuação: " + similarityScore + ", distância: " + euclideanDistance + ")" + (ssimWarning ? " [Aviso: SSIM baixo: " + ssimScore + "]" : "");
        }
        String detailedReason = facialReason;
        if (!areCoordinatesEqual) {
            fraudType = "Coordenadas Diferentes" + (areFacesEqual ? "" : " e " + fraudType);
            detailedReason += " [Fraude nas coordenadas: latDiff=" + latDiff + ", lonDiff=" + lonDiff + (FORCE_FRAUD_TEST ? ", coordenadas forçadas para teste de fraude]" : "]");
        } else if (areFacesEqual) {
            fraudType = "Sem Fraude";
            detailedReason = "Nenhuma fraude bloqueada: faces similares e coordenadas consistentes.";
        }
        // logger.info("Comparação facial: similarityScore={}, euclideanDistance={}, areFacesEqual={}, ssimScore={}, ssimWarning={}", similarityScore, euclideanDistance, areFacesEqual, ssimScore, ssimWarning);
        // logger.info("Comparação de coordenadas: areCoordinatesEqual={}, latDiff={}, lonDiff={}, FORCE_FRAUD_TEST={}", areCoordinatesEqual, latDiff, lonDiff, FORCE_FRAUD_TEST);
        ...
        neutralImg.release();
        smileImg.release();
        return new ImageComparisonResult(areFacesEqual, areCoordinatesEqual, similarityScore, detailedReason, lat1, lon1, lat2, lon2, euclideanDistance, fraudType);
    }
    ...
    private static FaceDetectionResult detectAndProcessFace(Mat img, String filePrefix) {
        ...
        Mat blob = Dnn.blobFromImage(resizedImg, 1.0, new Size(targetWidth, targetHeight), new Scalar(104, 117, 123), true, false);
        ...
        for (int i = 0; i < numDetections; i++) {
            ...
            if (confidence > CONFIDENCE_THRESHOLD && confidence > bestConfidence) {
                ...
                if (rect.width > 100 && rect.height > 100 && rect.x + rect.width <= originalCols && rect.y + rect.height <= originalRows) {
        ...
        blob.release();
        if (bestRect == null) {
            // logger.warn("Nenhuma face exibida com confiança > {}", CONFIDENCE_THRESHOLD);
            return new FaceDetectionResult(null, null, null, null);
        }
        // logger.debug("Face exibida com maior confiança ({}) em: {{{}, {}, {}x{}}}", bestConfidence, bestRect.x, bestRect.y, bestRect.width, bestRect.height);
        ...
        if (finalSize <= 0 || paddedX < 0 || paddedY < 0 || paddedX + finalSize > originalCols || paddedY + finalSize > originalRows) {
        ...
        String alignPath = logsPath + "aligned_" + filePrefix + ".jpg";
    ...
    private static Mat alignFace(Mat face, Rect faceRect) {
        ...
        if (cropRect.x + cropRect.width > faceWidth || cropRect.y + cropRect.height > faceHeight) {
            // logger.warn("Falha ao centralizar face: cropRect inválido x={}, y={}, width={}, height={}", cropRect.x, cropRect.y, cropRect.width, cropRect.height);
            Imgproc.resize(face, aligned, new Size(targetSize, targetSize));
        } else {
        ...
        // logger.debug("Face exibida: width={}, height={}", aligned.cols(), aligned.rows());
        return aligned;
    }
    ...
    private static Mat normalizeFace(Mat face) {
        ...
        // logger.debug("Face normalizada: canais={}, média={}", normalized.channels(), Core.mean(normalized));
        return normalized;
    }
    ...
    private static double calculateSSIM(Mat img1, Mat img2) {
        ...
        double ssim = ((2 * mu1 * mu2 + C1) * (2 * sigma12 + C2)) / ((mu1 * mu1 + mu2 * mu2 + C1) * (sigma1 * sigma1 + sigma2 * sigma2 + C2));
        resized1.release();
        resized2.release();
        gray1.release();
        gray2.release();
        ...
        temp2.release();
        multMat.release();
        // logger.debug("SSIM calculado: {}", ssim);
    ...
    private static double calculateEuclideanDistance(float[] emb1, float[] emb2) {
        if (emb1 == null || emb2 == null || emb1.length != emb2.length) {
            // logger.error("Embeddings inválidos ou de tamanhos diferentes: emb1={}, emb2={}", emb1 == null ? "null" : emb1.length, emb2 == null ? "null" : emb2.length);
